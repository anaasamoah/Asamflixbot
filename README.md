...
Or use the included PowerShell helper which stops stray python processes, sets the token for the session, and optionally tails `bot.log`:

```powershell
# Preferred on Windows when developing
.\run_bot.ps1 -Token "123456:ABC-DEF..." -Tail
# or if you already exported TELEGRAM_BOT_TOKEN in your session:
.\run_bot.ps1 -Tail
```
