...
# If the user runs this file from an editor that uses a different
# interpreter (common with "Run" buttons), automatically re-launch
# the script using the project's virtualenv Python if present. This
# makes running the bot from VS Code / editor Run buttons work
# without extra configuration.
def _maybe_relaunch_with_venv():
    try:
        project_root = os.path.dirname(__file__)
        venv_python = os.path.join(project_root, '.venv', 'Scripts', 'python.exe')
        if os.path.exists(venv_python):
            current = os.path.abspath(sys.executable)
            venvp = os.path.abspath(venv_python)
            # On Windows file-system comparisons should be case-insensitive
            if os.path.normcase(current) != os.path.normcase(venvp):
                print(f"Detected different Python interpreter ({current}). Relaunching with project venv: {venvp}")
                # Load .env into the child process environment if present
                env = os.environ.copy()
                ...
                if os.path.exists(env_file):
                    with open(env_file, 'r', encoding='utf-8') as f:
                        for ln in f:
                            ...
                            if k:
                                env[k] = v
                # Launch venv Python as a child process and exit the current process.
                # Using subprocess avoids command-line splitting issues on Windows
                # that can occur with execv when paths contain spaces.
                subprocess.Popen([venvp] + sys.argv, env=env)
                sys.exit(0)
    except Exception as _e:
        # Don't fail startup if relaunch fails; continue and let diagnostics show problems.
        print('Relaunch attempt failed:', _e)


_maybe_relaunch_with_venv()

# Early diagnostic: if the `telegram` package isn't available in the
# interpreter that's running this script, emit a clear, actionable
# message and exit. Editors/IDE "Run" buttons often use a different
# Python than your project venv which causes ModuleNotFoundError.
try:
    import telegram
    from telegram import Update
    from telegram.ext import ContextTypes
except Exception as e:
    print("ERROR: Missing required package 'python-telegram-bot' (module 'telegram')")
    print("Interpreter:", sys.executable)
    print("Python sys.path:\n", "\n".join(sys.path))
    print()
    print("To fix:")
    print("  1) Activate your project venv in PowerShell:")
    print("     .\\.venv\\Scripts\\Activate.ps1")
    print("  2) Install dependencies:")
    print("     pip install -r requirements.txt")
    print("  3) Run the bot with the venv python:")
    print("     & \".\\.venv\\Scripts\\python.exe\" .\\main.py")
    print()
    print("If you want me to run these steps for you, say 'run install'.")
    # Also print the original exception for quick debugging
...
def validate_and_fix_json(filename: str, expected_type: type, default):
    """Ensure `filename` contains valid JSON of `expected_type`.
    If the file is corrupt or the type is wrong, rename it to a .bad.TIMESTAMP
    and write a safe default so the bot can continue.
    Returns True if file was OK or was created/fixed, False on unrecoverable error.
    """
    logger = logging.getLogger('asamflixbot')
    if not os.path.exists(filename):
        try:
            save_json(filename, default)
            logger.info("Created missing data file: %s", filename)
            return True
        except Exception as e:
            print(f"Failed to create missing {filename}: {e}")
            return False
    ...
    try:
        ...
        if not isinstance(data, expected_type):
            raise ValueError(f"Expected {expected_type} in {filename}, got {type(data)}")
        return True
    except Exception as e:
        # Backup the corrupt file and replace with default
        try:
            badname = f"{filename}.bad.{int(time.time())}"
            os.rename(filename, badname)
            logger.warning("Backed up corrupt %s -> %s (%s)", filename, badname, e)
        except Exception as ee:
            logger.error("Failed to backup corrupt file %s: %s", filename, ee)
        try:
            save_json(filename, default)
            logger.info("Wrote safe default for %s", filename)
            return True
        except Exception as ee:
            logger.error("Failed to write safe default for %s: %s", filename, ee)
            return False


def validate_data_files():
    # Map filename -> (expected_type, default_value)
    checks = {
        RESPONSES_FILE: (dict, {}),
        MOVIE_FILES: (dict, {}),
        WARNINGS_FILE: (dict, {}),
    ...
    ok = True
    for fname, (typ, default) in checks.items():
        if not validate_and_fix_json(fname, typ, default):
            ok = False
    return ok

# Validate data files and recover from corrupt JSON before loading
try:
    validate_data_files()
except Exception:
    # If validation unexpectedly fails, continue and let load_json handle missing files
    pass
...
async def warn_and_block(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Defensive: ensure message and text exist
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Defensive handling: not all Update objects include a Message
    # and some messages contain no text (e.g. only a document). Avoid
    # accessing `update.message.text` when it's None to prevent
    # AttributeError: 'NoneType' object has no attribute 'text'.
    msg = getattr(update, 'message', None)
    if msg is None:
        return
    message_text = getattr(msg, 'text', None)
    # If there's no text payload, ignore here; attachments are
    # handled by `handle_document` (registered for filters.ATTACHMENT).
...
def make_genre_handler(genre):
    # Return an async handler that awaits the async genre_command.
    async def _handler(update, context):
        await genre_command(update, context, genre)
    return _handler
...
async def error(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Log full exception with traceback for later inspection
    try:
        logging.getLogger('asamflixbot').exception('Exception while handling update: %s', update)
    except Exception:
        # fallback
        try:
    ...
    # Try to notify admins (best-effort, do not raise)
    try:
        err_text = str(context.error)
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(admin_id, f"Bot error: {err_text}")
            except Exception:
                pass
    except Exception:
        pass
...
if __name__ == '__main__':
    ...
    # PID lockfile to prevent accidental duplicate local runs
    def _pidfile_path():
        return os.path.join(os.path.dirname(__file__), '.asamflixbot.pid')

    def _process_exists(pid: int) -> bool:
        try:
            # Try a cross-platform check: on Unix use os.kill(0), on Windows use tasklist
            if os.name == 'nt':
    ...
    def _write_pidfile():
        ...
        try:
            ...
            # If we cannot write a pidfile, continue but warn
            print('Warning: could not write pidfile', path)
        # Ensure pidfile cleanup on normal exit
        try:
            atexit.register(_remove_pidfile)
        except Exception:
            pass

    # If a PID lockfile exists and the process is alive, refuse to start.
    try:
        ...
        if os.path.exists(pidfile):
            ...
            if other_pid and _process_exists(other_pid) and os.environ.get('FORCE_START', '') != '1':
                print(f"Another bot process appears to be running (PID={other_pid}). Refusing to start to avoid duplicate polling.\nIf you are sure no other instance is running, remove {pidfile} or set environment variable FORCE_START=1 to override.")
                sys.exit(3)
            else:
        ...
        # If PID checks fail for any reason, do not block startup.
        pass

    # Write our pidfile for others to see
    try:
        _write_pidfile()
    except Exception:
        pass

    # Defensive: prevent accidental execution when editor runs a temporary runner file
    # Some editors create a temp file (e.g. tempCodeRunnerFile.py) which may contain only
    # a fragment or a single character and will cause confusing errors. Refuse to run
    # when that is detected so users run `main.py` directly.
    try:
        runner_name = os.path.basename(sys.argv[0]).lower() if len(sys.argv) > 0 else ''
        if 'tempcoderunnerfile' in runner_name or 'tempcoderunner' in runner_name:
            print("Refusing to run: detected editor temp runner. Run `main.py` directly.")
            sys.exit(1)
    except Exception:
        # If anything goes wrong checking argv, continue normally
        pass
    ...
    def build_application():
        """Construct and return a ready-to-run Application instance."""
        ...
        try:
            ...
            # Continue even if we can't patch Application
            pass

        builder = ApplicationBuilder().token(TOKEN)
        # If the telegram JobQueue implementation tries to store a weakref
        # to Application but the Application class is not weakref-able
        # (TypeError: cannot create weak reference), monkeypatch
        # JobQueue.set_application to store a callable that returns the
        # application instead. This mirrors the weakref.ref interface
        # (it's callable) and avoids modifying telegram internals.
        try:
            import telegram.ext._jobqueue as _jq
            _orig_set_application = getattr(_jq.JobQueue, 'set_application', None)
            def _set_application_no_weak(self, application):
                try:
                    # store a callable returning the application (like weakref.ref)
                    self._application = (lambda: application)
                except Exception:
                    if _orig_set_application:
                        _orig_set_application(self, application)
            _jq.JobQueue.set_application = _set_application_no_weak
        except Exception:
            # If monkeypatching fails, continue and let builder.build() attempt normally
            pass
        try:
        ...
        # JobQueue scheduled posting
        try:
            if getattr(app, 'job_queue', None) is not None:
                app.job_queue.run_repeating(scheduled_posting_job, interval=60, first=10)
            else:
                logger.info("JobQueue not available; scheduled posting disabled.")
        except Exception as e:
            logger.exception("Could not schedule jobs; continuing without scheduled posting")
    ...
    def run_forever(restart_delay: int = 5):
        """Run the bot, restart on unexpected crashes, and log errors."""
        logger.info('Starting asamflixbot runner')
        while True:
            try:
                app = build_application()
                logger.info('Built application; entering polling')
                print('polling...')
                sys.stdout.flush()
                app.run_polling(allowed_updates=None)
                # If run_polling returns normally, that's unexpected; restart
                logger.warning('app.run_polling() returned; restarting after delay')
            except KeyboardInterrupt:
                logger.info('Bot stopped by user (KeyboardInterrupt)')
                print('\nBot stopped by user (Ctrl+C).')
                sys.exit(0)
            except Exception as e:
                # If Telegram reports a Conflict (another getUpdates caller),
                # it's better to exit so we don't repeatedly log the same error
                # and so you can start a single instance from your side.
                tb = traceback.format_exc()
                if 'terminated by other getUpdates request' in tb or 'terminated by other getUpdates' in tb or 'Conflict' in str(e):
                    logger.error('Detected Telegram Conflict: another getUpdates instance is running. Exiting to allow single-instance takeover.')
                    try:
                        for admin_id in ADMIN_IDS:
                            try:
                                # best-effort admin notification
                                if 'app' in locals() and getattr(app, 'bot', None) is not None:
                                    # use app.bot if available
                                    import asyncio
                                    asyncio.get_event_loop().create_task(app.bot.send_message(admin_id, 'Bot stopped due to Conflict: another instance is running.'))
                            except Exception:
                                pass
                    except Exception:
                        pass
                    # Exit without trying to restart; user can start their preferred instance.
                    sys.exit(2)

                logger.exception('Unhandled exception in bot; will restart')
                # give logs time to flush and avoid tight crash loops
                try:
                    # small backoff to avoid tight crash loops
                    time.sleep(restart_delay)
                except Exception:
                    # if sleep fails for any reason, continue immediately
                    pass
                # loop will continue and attempt rebuild/restart

    # Set a global exception hook so unhandled exceptions get logged
    def _excepthook(exc_type, exc_value, exc_tb):
        logger.error('Uncaught exception', exc_info=(exc_type, exc_value, exc_tb))

    sys.excepthook = _excepthook

    # Also set asyncio exception handler
    try:
        import asyncio
        loop = asyncio.get_event_loop()
        def _async_exc(loop, context):
            logger.error('Asyncio exception: %s', context)
        loop.set_exception_handler(_async_exc)
    except Exception:
        pass
